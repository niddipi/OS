...
public class Fault_Handlers{
   ...
   {
      m_util.pcb[0].Program_seg_info  = 
	 Prog_Page_Fault_Handler(jobid,no_of_pages); 
      m_util.pcb[jobid].Program_seg_info=jobid;
      return jobid;
   }
   ...
   public int Prog_Page_Fault_Handler(int jobid , int no_of_pages)
   {
	...
	if(m_util.pcb[0].Program_seg_info == -1)
	{
      		Address_of_PMT = jobid * no_of_pages;
		m_util.pcb[0].Page_Mem_order[Address_of_PMT].no_of_pages =no_of_pages; 
		
	}
	else
	{
		Address_of_PMT = m_util.pcb[0].Program_seg_info;
		Address_of_PMT = 
   ...
   {
        m_util.pcb[jobid].Output_seg_info = Output_Page_Fault_Handler(jobid); 
	util.CLOCK = util.CLOCK + 5;
	util.SEG_FAULT_CLOCK = util.SEG_FAULT_CLOCK + 5;
	 if((util.CLOCK - util.OLD_CLOCK) >= 15)
	{
   ...
   {
	int Address_of_PMT = 0;
        if(m_util.pcb[jobid].Output_seg_info == -1)
        {
                 Address_of_PMT = jobid;
        ...
        {
                Address_of_PMT = m_util.pcb[jobid].Output_seg_info;
                Address_of_PMT = 
                        new Page_Replacement().Page_Replacement_func(Address_of_PMT);
